...
// Gemini API key — used by the Map 3D component. May be unavailable during auth/onboarding.
...
function App() {
  return (
    <AuthProvider>
      <AuthGuard>
        {(screen) => <AppRouter screen={screen} />}
      </AuthGuard>
    </AuthProvider>
  );
}
...
function AppRouter({ screen }: { screen: AppScreen }) {
  ...
  // screen === 'main' → Mapa 3D original
  return (
    <div className="App">
      <APIProvider
        version={'alpha'}
        apiKey={import.meta.env.VITE_GOOGLE_MAPS_API_KEY}
        solutionChannel={"gmp_aistudio_mundoaimas_v1.0.0"}>
        <AppComponent />
      </APIProvider>
    </div>
  );
